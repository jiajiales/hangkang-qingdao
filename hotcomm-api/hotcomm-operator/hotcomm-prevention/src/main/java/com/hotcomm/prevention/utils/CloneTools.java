...
public class CloneTools {
	...
	public static <T> T cloneObj(Object obj, Class<T> toResult) {
		...
		try {
			...
			for (Field field : objfields) {
				list.add("get" + field.getName().substring(0, 1).toUpperCase() + field.getName().substring(1));
			}
			for (Field field : fields) {
				...
				if (isWrapType(field)) {
					...
					for (String getName : list) {
						if (getMethodName.equals(getName)) {
							test = 1;
							break;
						}
					}
					if (test == 1) {
						...
						// 如果get 和 set方法都从一个对象中获取会出现object is not an instance of
						// declaring class这个错误
						// like: User{name} People{name}
						// 因为如果从源对象中获取，在setMethod.invoke调用的时候，虽然名字相同，会由于类的不同，导致
						// 调用的方法却不是目标对象中应有的方法。实际就是:getMethod =
						// com.package.User.getName();setMethod =
						// com.package.User.setName();
						// 而setMethod.invoke调用的结果就变成 People.setName() ==
						// People.(com.package.User.setName())
						...
						setMethod.invoke(t, new Object[] { value }); // set
																		// 设置的是目标对象的值
					} else {
						continue;
					}
	...
	private static boolean isWrapType(Field field) {
		 ...
		 */
		public static <T>T combineSydwCore(Object sourceBean, Object targetBean) {
