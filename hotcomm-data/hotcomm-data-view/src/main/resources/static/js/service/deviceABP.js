var toolbars = {
    ...
    "device-list-import-temp-download": "下载ABP设备导入模板",
...
//    $('#groupId').combobox({width:352});
    ...
    var grid_columns = [
        [{
            ...
            field: 'code', title: 'DevAddr', width: 25, align: 'center',
            ...
            field: 'nwkSKey', title: 'NwkSKey', width: 25, align: 'center', formatter: formatTitle
        }, {
...
//            field: 'receiveNum', title: '接收数据数量', width: 20, align: 'center', formatter: formatTitle
            ...
            formatter: function (value, row, index) {
                return authToolBar({
                	"device-get": '<span data-code="' + row.code + '" class="ctr fa-info-circle device-lora-abp-get-receiveNum">' + toolbars["device-get"] + '</span>',
                    "device-edit": '<span data-id="' + row.deviceId + '" class="ctr ctr-edit device-lora-abp-update">' + toolbars["device-edit"] + '</span>',
                    "device-delete": '<span data-id="' + row.deviceId + '" class="ctr ctr-delete device-lora-abp-del">' + toolbars["device-del"] + '</span>'
                }).join("");
            }
        }]
    ]
    ...
    var datagrid = {
        ...
        toolbar: authToolBar({
            ...
            "device-list-import-temp-download": {
                ...
                handler: function () {
                	$(location).attr('href', "static/download/device/ABP.xlsx");
                }
            },
            "device-list-import": {
            	id: 'device-lora-abp-import',
    ...
    function devicePage() {
        ...
        $('#device_datagrid').datagrid({
            url: bindToken(fun_urls.device.lora_abp_page_url),
            queryParams: params
        });
    }
    ...
    }).on('click', ".device-lora-abp-get-receiveNum", function () { // 查看按钮事件
        var dglog = this;
    ...
    }).on('click', ".device-lora-abp-update", function () { // 编辑按钮事件
        var dglog = this;
        var dgId = this.dataset.id;
        $.post(bindToken(fun_urls.device.lora_abp_get_url), {"deviceId": this.dataset.id}, function (data) {
            if (data.code == "0") {
    ...
    }).on('click', ".device-lora-abp-del", function () { // 删除按钮事件
    ...
    function createForm(id) {
        var dialog = $("<div/>", {
            ...
            height: 400,
            width: fixWidth(0.35),
            href: bindDialog('/service/deviceABPAdd'),
            modal: true,
            ...
            buttons: [{
                ...
                handler: function () {
                    if (form.form('validate')) {
                        $.post(bindToken(fun_urls.device.lora_abp_add_url), form.serialize(), function (res) {
    ...
    function createUpdateForm(id, device) {
        ...
        var dialog = $("<div/>", {
            ...
            height: 445,
            width: fixWidth(0.35),
            href: bindDialog('/service/deviceABPUpdate'),
            queryParams: {
                id: id
            },
            modal: true,
            ...
            onLoad: function () {
                ...
                $(form).find('#nwkSKey').textbox("setValue", device.nwkSKey);
            ...
            buttons: [{
                ...
                handler: function () {
                    ...
                    if (form.form('validate')) {
                        ...
                        $.post(bindToken(fun_urls.device.lora_abp_update_url), paramToUpdate, function (res) {
                            if (res.code == "0") {
                                $("#device_datagrid").datagrid('reload');
                                $.messager.show({
                                	title: "编辑设备信息",
    ...
    function createDevImportForm() {
        var dialog = $("<div/>", {
            ...
            onLoad: function () {
                ...
                $(form).find('#deviceGroupIdForImport').combobox({
                    label: "选择设备组：",
                    textField: 'text',
                    url: bindToken(fun_urls.deviceGroup.list_url),
                    editable: false,
            ...
            buttons: [{
                ...
                handler: function () {
                    if (form.form('validate')) {
                        ...
                        $.ajax({
                            url: bindToken(fun_urls.device.lora_abp_import_url),
                            type: 'POST',
                            data: formData,
                            async: false,
                            cache: false,
    ...
    function createDevExportForm() {
        ...
        var dialog = $("<div/>", {
            ...
            buttons: [{
                ...
                text: '导出',
                handler: function () {
                	$(location).attr('href', bindToken(fun_urls.device.lora_abp_export_url) + "&devGroupId=" + groupIdForExport);
                    $.messager.show({
                    	title: "导出设备列表",
                        msg: "导出成功",
                        timeout: 3000
    ...
    function createViewForm(code) {
        ...
        var dialog = $("<div/>", {
            ...
            title: "查看相关数据【设备硬件编号 - DevAddr : " + code + "】",
